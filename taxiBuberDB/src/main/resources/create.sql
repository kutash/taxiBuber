...
-- -----------------------------------------------------
-- Table `taxi_buber`.`country`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `taxi_buber`.`country` (
  `id_country` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'country id',
  `name` VARCHAR(100) NOT NULL COMMENT 'country title',
...
CREATE TABLE IF NOT EXISTS `taxi_buber`.`address` (
  ...
  `city` VARCHAR(45) NOT NULL COMMENT 'name of the locality',
  `street` VARCHAR(45) NOT NULL COMMENT 'name of the street',
  `house` VARCHAR(45) NULL DEFAULT NULL COMMENT 'number of the house, optional field',
  `flat` VARCHAR(45) NULL DEFAULT NULL COMMENT 'number of the flat, optional field',
  `type` ENUM('HOME', 'WORK', 'OTHER') NULL DEFAULT NULL COMMENT 'type of the address, one of 3 values',
  `latitude` CHAR(10) NOT NULL COMMENT 'geographical latitude, 9 characters or 10 for negative values',
  `longitude` CHAR(10) NOT NULL COMMENT 'geographic longitude, 9 characters or 10 for negative values',
  `id_user` INT(10) UNSIGNED NOT NULL COMMENT 'each address must have a specific user',
  `id_country` INT(10) UNSIGNED NOT NULL COMMENT 'country id',
  PRIMARY KEY (`id_address`),
  INDEX `fk_address_user1_idx` (`id_user` ASC),
  INDEX `fk_address_country1_idx` (`id_country` ASC),
  CONSTRAINT `fk_address_country1`
  FOREIGN KEY (`id_country`)
  REFERENCES `taxi_buber`.`country` (`id_country`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `taxi_buber`.`car` (
  ...
  `latitude` CHAR(10) NOT NULL COMMENT 'geographical latitude of the car at time of order',
  `longitude` CHAR(10) NOT NULL COMMENT 'eographical longitude of the car at time of order',
  `id_brand` INT(10) UNSIGNED NOT NULL COMMENT 'car brand id',
  `id_user` INT(10) UNSIGNED NOT NULL COMMENT 'car owner\'s id',
  PRIMARY KEY (`id_car`, `id_brand`, `id_user`),
  UNIQUE INDEX `state_number_UNIQUE` (`registration_number` ASC),
  INDEX `fk_car_user_idx` (`id_user` ASC),
...
CREATE TABLE IF NOT EXISTS `taxi_buber`.`trip` (
  ...
  `status` ENUM('ORDERED', 'ACCEPTED', 'COMPLETED') NOT NULL COMMENT 'status of the trip, one of 3 values',
